...
namespace AutoMicroMapper.MicroMapper.UnitTests
{
	...
	{
		public class When_mapping_two_non_configured_types : SpecBase
		{
			...
			{
				_resultWithGenerics = MicroMapper.MicroMapper.DynamicMap<Source, Destination>(new Source {Value = 5});
				_resultWithoutGenerics = (Destination) MicroMapper.MicroMapper.DynamicMap(new Source {Value = 5}, typeof(Source), typeof(Destination));
			}
		...
		public class When_mapping_two_non_configured_types_that_do_not_match : NonValidatingSpecBase
		{
			...
			{
				var destination = MicroMapper.MicroMapper.DynamicMap<Source, Destination>(new Source {Value = 5});
			...
			{
				typeof(ConfigurationException).ShouldNotBeThrownBy(() => MicroMapper.MicroMapper.DynamicMap<Source, Destination>(new Source { Value = 5 }));
			}
		}
		...
		public class When_mapping_to_an_existing_destination_object : NonValidatingSpecBase
		{
			...
			protected override void Because_of()
			{
				...
				MicroMapper.MicroMapper.DynamicMap(new Source { Value = 5, Value2 = 3}, _destination);
			}
		...
		public class When_mapping_from_an_anonymous_type_to_an_interface : SpecBase
		{
			...
			protected override void Because_of()
			{
				_result = MicroMapper.MicroMapper.DynamicMap<IDestination>(new {value = 5});
			}
